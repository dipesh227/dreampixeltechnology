...
const generateWithPerplexity = async (): Promise<string | null> => {
    throw new Error("Perplexity API does not currently support the advanced image generation required by this tool. Please select 'Custom Gemini' or 'OpenRouter' in the API Settings for full functionality.");
};
...
`;
    try {
        ...
        throw new Error("Failed to generate thumbnail prompts. Please try again.");
    }
};
...
`;
    ...
    if (config.provider === 'openrouter') {
         return generateWithOpenRouter(enhancedPrompt, headshots);
    }
    ...
    const ai = getAiClient();
    try {
        const imageParts = headshots.map(file => ({
        ...
        throw new Error("Failed to generate the thumbnail image. The AI model may be overloaded or the prompt might be too complex. Please try again later or with a different concept.");
    }
};


export const generatePosterPrompts = async (
...
`;
    try {
        ...
        if (result && result.concepts && Array.isArray(result.concepts)) {
            ...
            throw new Error("Invalid response format from AI for poster prompts.");
        }
    } catch (error) {
        console.error("Error generating poster prompts:", error);
        throw new Error("Failed to generate poster concepts. Please try again.");
    }
};
...
`;
    ...
    if (config.provider === 'openrouter') {
        return generateWithOpenRouter(finalPrompt, headshots);
    }
     if (config.provider === 'perplexity') {
        return generateWithPerplexity();
    }
    ...
    try {
        ...
        console.error("Error generating political poster:", error);
        throw new Error("Failed to generate the poster. The AI model may be overloaded. Please try again later.");
    }
};
...
`;
    try {
        ...
        if (result && result.concepts && Array.isArray(result.concepts)) {
            ...
            throw new Error("Invalid response format from AI for ad concepts.");
        }
    } catch (error) {
        console.error("Error generating ad concepts:", error);
        throw new Error("Failed to generate ad concepts. Please try again.");
    }
};

export const generateAdBanner = async (
    selectedPrompt: string,
    productImage: UploadedFile,
    modelHeadshot: UploadedFile,
    headline: string,
    brandDetails: string,
    aspectRatio: AspectRatio
): Promise<string | null> => {
...
`;
    ...
    if (config.provider === 'openrouter') {
        return generateWithOpenRouter(finalPrompt, allImages);
    }
     if (config.provider === 'perplexity') {
        return generateWithPerplexity();
    }
    ...
    try {
        ...
        throw new Error("Failed to generate the ad banner. The AI model may be overloaded. Please try again later.");
    }
};
