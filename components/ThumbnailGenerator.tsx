...
const ThumbnailGenerator: React.FC<ThumbnailGeneratorProps> = ({ onNavigateHome, onThumbnailGenerated }) => {
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl">
                 ...
                 <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl space-y-4">
                    ...
                    <input type="text" value={thumbnailText} onChange={e => setThumbnailText(e.target.value)} placeholder="e.g., 'AI TAKEOVER?!' or 'My BIGGEST Secret'" className="w-full p-3 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600 transition text-sm" />
                    ...
                    <input type="text" value={brandDetails} onChange={e => setBrandDetails(e.target.value)} placeholder="e.g., 'DreamPixel Tech'" className="w-full p-3 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-slate-600 focus:border-slate-600 transition text-sm" />
                </div>
            </div>
            ...
            <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <button onClick={() => setAspectRatio('9:16')} className={`flex flex-col items-center justify-center p-6 rounded-lg border-2 transition-colors duration-200 ${aspectRatio === '9:16' ? 'border-blue-500 bg-slate-800/50' : 'border-slate-800 bg-slate-900 hover:border-slate-700'}`}>
             ...
             <div className="flex justify-center pt-4">
                <button onClick={handleGeneratePrompts} disabled={isLoading || !description.trim()} className="flex items-center gap-3 px-8 py-4 bg-slate-200 text-slate-900 font-bold text-lg rounded-lg hover:bg-white transition-all duration-300 disabled:bg-slate-800 disabled:text-slate-500 disabled:cursor-not-allowed transform hover:scale-105">
    ...
    const renderResultStep = () => (
        <div className="max-w-4xl mx-auto text-center animate-fade-in">
             ...
             <div className="flex justify-center gap-4 flex-wrap">
                 ...
                 <button onClick={() => handleGenerateThumbnail(finalPrompt)} disabled={isLoading} className="flex items-center gap-2 px-6 py-3 bg-slate-800 text-white font-bold rounded-lg hover:bg-slate-700 transition-all duration-300 border border-slate-700 disabled:opacity-60 disabled:cursor-not-allowed">
                 ...
                 <a href={`data:image/png;base64,${generatedThumbnail}`} download="dreampixel-thumbnail.png" className="flex items-center gap-2 px-6 py-3 bg-slate-200 text-slate-900 font-bold rounded-lg hover:bg-white transition-all duration-300 transform hover:scale-105">
