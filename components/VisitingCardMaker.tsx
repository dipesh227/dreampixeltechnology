...
import { HiArrowDownTray, HiOutlineHeart, HiOutlineSparkles, HiArrowUpTray, HiXMark, HiOutlineBuildingOffice2, HiOutlineUser, HiOutlineIdentification, HiOutlineEnvelope, HiOutlineArrowPath, HiArrowLeft, HiOutlineDocumentDuplicate, HiCheck, HiOutlineLightBulb, HiOutlineQueueList } from 'react-icons/hi2';
...
const VisitingCardMaker: React.FC<VisitingCardMakerProps> = ({ onNavigateHome, onCreationGenerated, onGenerating }) => {
    ...
    useEffect(() => {
        ...
        if (isLoading && step === 'input') {
            const messages = ['Analyzing brand details...', 'Sketching card layouts...', 'Choosing typography...'];
            let index = 0;
            ...
            const messages = ['Initializing design vectors...', 'Rendering card details...', 'Applying color palettes...', 'Finalizing business card...'];
            let index = 0;
            setLoadingMessage(messages[index]);
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.files && event.target.files.length > 0) {
            ...
            reader.onload = () => {
                ...
                setLogo({ base64, mimeType: file.type, name: file.name });
            };
    ...
    const handleGenerateConcepts = async () => {
        if (!companyName.trim() || !personName.trim() || !title.trim() || !contactInfo.trim()) {
            setError('Please fill in all required fields: Company Name, Your Name, Title, and Contact Info.');
            return;
        }
        ...
        setError(null);
        try {
            const selectedStyle = VISITING_CARD_STYLES.find(s => s.id === selectedStyleId);
            if (!selectedStyle) throw new Error("A style must be selected.");
            
            const prompts = await generateVisitingCardPrompts(companyName, personName, title, contactInfo, selectedStyle, !!logo);
    ...
    const renderInputStep = () => (
        <div className="space-y-8">
                ...
                <div className="p-4 md:p-6 bg-slate-900/60 backdrop-blur-lg border border-slate-700/50 rounded-xl" data-tooltip="Optionally upload your company logo. A transparent PNG file is recommended for best results.">
                    ...
                    <div className="p-6 border-2 border-dashed border-slate-700 rounded-xl text-center bg-slate-800/50 hover:border-slate-600 transition h-40 flex flex-col justify-center">
                         ...
                         <label htmlFor="file-upload" className="cursor-pointer">
                            ...
                            <p className="text-slate-300 font-semibold text-sm">Upload Your Logo</p>
                         </label>
                    </div>
                     {logo && 
                        <div className="flex justify-center mt-2">
                            <div className="relative group w-20 h-20 bg-white p-1 rounded-lg">
                                <img src={`data:${logo.mimeType};base64,${logo.base64}`} alt={logo.name} className="object-contain w-full h-full"/>
                                <button onClick={() => setLogo(null)} className="absolute top-1 right-1 bg-black/60 rounded-full p-1 text-white hover:bg-red-500 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            ...
            <div className="p-4 md:p-6 bg-slate-900/60 backdrop-blur-lg border border-slate-700/50 rounded-xl" data-tooltip="Select a style to define the visual direction of your visiting card.">
                 <h2 className="text-xl font-bold text-white mb-4">3. Choose a Design Style</h2>
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {VISITING_CARD_STYLES.map((style: VisitingCardStyle) => (
    ...
    const renderPromptSelectionStep = () => (
        <div className="max-w-7xl mx-auto animate-fade-in">
            <h2 className="text-3xl font-bold text-center mb-2 text-white">Choose Your Card Concept</h2>
            <p className="text-slate-400 text-center mb-10">Select a concept below to generate your final design.</p>
    ...
    const renderResultStep = () => (
        <div className="max-w-4xl mx-auto text-center animate-fade-in">
             <h2 className="text-3xl font-bold text-center mb-8 text-white">Your Visiting Card is Ready!</h2>
             {generatedImage && (
                <div className="inline-block p-2 bg-slate-800 rounded-xl">
                    <img src={`data:image/png;base64,${generatedImage}`} alt="Generated Visiting Card" className="max-w-md w-full h-auto mx-auto shadow-2xl shadow-black/30 rounded-lg" style={{ aspectRatio: '3.5 / 2' }} />
                </div>
             )}
